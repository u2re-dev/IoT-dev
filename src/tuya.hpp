...
public: 
    ...
    TuyaDevice3(SCREEN* _screen_, char const* prefix) : _screen_(_screen_) {
        ...
        protocol33.setKey(_concat_(prefix, "_pro3"));
        ENABLED.setKey(_concat_(prefix, "_en"));
    }
...
protected: 
    ...
    //
    _NvString_<22> _device_id_;
    _NvString_<16> _real_local_key_;
    nv_uint32_t _IP_ADDRESS_; // in reality, is packed 8x4 bits
    nv_bool protocol33;
...
public: 
    ...
    uint32_t handleReceive() {
        ...
        //
        if (client.connected()) {
            auto [_len_, _received_] = receive(client, (20 + 4 + (_hmac_key_ ? 32 : 4)));
    ...
    //#endif
                        ...
                        if (!_sending_) { _sending_ = (uint8_t*)calloc(1, LIMIT); };
                        encodeMessage((uint8_t*)_sending_, 5, _remote_hmac_, hlen, _hmac_key_);
                        send(client, (uint8_t const*)_sending_, calculateSizeOfRequest(hlen, _hmac_key_));
                        //lastTime = millis();
