...
namespace tc {
    ...
    //
    inline uint32_t getTuyaCmd(uint8_t* encrypted_code) {
        return bswap32(*(uint32_t const*)(encrypted_code+8));
    }

    //
    inline uint32_t getTuyaSeq(uint8_t* encrypted_code) {
        return bswap32(*(uint32_t const*)(encrypted_code+4));
    }

    //
    inline uint8_t* getTuyaPayload(uint8_t* encrypted_code, size_t& length) {
        length = bswap32(*(uint32_t const*)(encrypted_code+12));
        return (encrypted_code+20);
    }

    //
    inline uint32_t computePayloadSize(uint32_t payloadLen, bool hmac) {
        return payloadLen + (hmac ? 32 : 4) + 4;
    }

    //
    inline uint32_t computeCodeSize(uint32_t payloadLen, bool hmac) {
