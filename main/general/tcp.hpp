...
public: ~TCPClient() { close(sockfd); }
    inline int connectToServer(const std::string& ip_address, int port) {
        ...
        if (inet_pton(AF_INET, ip_address.c_str(), &server_addr.sin_addr) <= 0)
            { std::cerr << "Error: Invalid IP address format: " << ip_address << std::endl; close(sockfd); return 1; }

        //
        if (connect(sockfd, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0)
            { std::cerr << "Error: Unable to connect to server at " << ip_address << ":" << port << std::endl; close(sockfd); return 1; }
    ...
    inline bytespan_t receiveResponse() {
        ...
        //
        if (received < 0)
            { std::cerr << "Error: Failed to receive message." << std::endl; return {}; }
        (*buffer)[received] = '\0';
