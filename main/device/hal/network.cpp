...
//
int connectToDevice(WiFiClient& client, IPAddress const& local, uint16_t port) {
    const auto start = millis();
...
//
void waitAndSend(WiFiClient& client, uint8_t* data, size_t length) {
...
//
bool waitForReceive(WiFiClient& client, uint8_t* data, size_t& length, size_t timeout) {
    bool done = false;
