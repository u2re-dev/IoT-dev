...
public:
    ...
    inline uint8_t handlePAKE3(Payload const& payload) {
        ...
        if (hkdf.hAY != bigint_t(tlv.find(01)->data())) { throw std::runtime_error("hAY not match in MAKE3 phase (received value)"); }
    ...
    inline uint8_t handlePAKE1(Payload const& payload) {
        ...
        //
        auto tlv = tlvcpp::tlv_tree_node{}; tlv.deserialize(*payload.payload);
        X_ = spake->parseECP(tlv.find(01)->data());
    ...
    inline bytespan_t makePAKE2(Message const& request) {
        ...
        //
        bytespan_t fda = spake->computeY();
        bigint_t hBX = spake->computeHKDFFromX(X_).hBX;
        resp.add_child(fda, 01);
        resp.add_child(hBX, 02);
