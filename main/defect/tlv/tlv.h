...
namespace tlvcpp {
    ...
    public: //
        ...
        //
        tlv(control_t const& control, tag_t const& tag = 0) : m_tag(tag), m_control(control) { TAGFIX; }
        ...
        //
        explicit tlv(uint8_t const* str, tag_t const& tag = 0) : m_tag(tag), m_size(std::strlen((char const* )str)), m_payload(str), m_control({ 0b00, e_type::UTF8_STRING, 0b00 }) { TAGFIX; }
        ...
        //
        tlv(bigint_t const& value, tag_t const& tag = 0) : m_size(32), m_tag(tag), m_payload((uint8_t const*)&value), m_control({0b00, e_type::BYTE_STRING, 0b000}) { TAGFIX; }
        tlv(intx::uint128 const& value, tag_t const& tag = 0) : m_size(16), m_tag(tag), m_payload((uint8_t const*)&value), m_control({0b00, e_type::BYTE_STRING, 0b000}) { TAGFIX; }
        ...
        tlv& operator=(bytespan_t const& v) { m_size = v->size(); m_payload = v->data(); m_control.type = e_type::BYTE_STRING; return *this; }

        //
        tlv& operator=(bigint_t const& v)      { m_size = 32; m_payload = reinterpret_cast<uint8_t const*>(&v); m_control.type = e_type::BYTE_STRING; m_control.octet = 0; return *this; }
        tlv& operator=(intx::uint128 const& v) { m_size = 16; m_payload = reinterpret_cast<uint8_t const*>(&v); m_control.type = e_type::BYTE_STRING; m_control.octet = 0; return *this; }
        ...
        tlv& setBytes(bytespan_t const& bytes) { m_size = bytes->size(); m_payload = bytes->data(); m_control.type = e_type::BYTE_STRING; m_control.octet = 0; return *this; }
