...
private:
    ...
    inline HKDF_HMAC computeHKDF( ecp_t X, ecp_t Y, ecp_t Z, ecp_t V) const {
        ...
        auto Ka = static_cast<intx::uint128>(transcript >> 0), Ke = static_cast<intx::uint128>(transcript >> 16);

        //
        auto info = hex::s2b("ConfirmationKeys");
        auto KcAB = crypto::hkdf(Ka, info); auto KcA = static_cast<intx::uint128>(KcAB >> 0), KcB = static_cast<intx::uint128>(KcAB >> 16);
