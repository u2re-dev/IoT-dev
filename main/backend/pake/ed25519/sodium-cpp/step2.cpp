...
const unsigned char response1[crypto_spake_RESPONSE1BYTES])
{
    ...
    //
    uint16_t ver; pop16(ver, stored_data, pos); if (ver != SER_VERSION) return -1;
    ...
    //
    memcpy(salt, stored_data + pos, crypto_pwhash_SALTBYTES);
    ...
    //
    memcpy(response2 + 32, validators.client_validator.data(), 32);
    memcpy(st.server_validator.data(), validators.server_validator.data(), 32);
    return 0;
}
#endif
