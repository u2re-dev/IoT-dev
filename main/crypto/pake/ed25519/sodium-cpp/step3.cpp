...
const unsigned char response2[crypto_spake_RESPONSE2BYTES])
{
    ...
    unsigned char V[32] = {0}, Z[32] = {0}, gy[32] = {0};
    ...
    //
    if (shared_keys_and_validators(shared_keys, validators, client_id, client_id_len, server_id, server_id_len, st.X.data(), Y, Z, st.h_K.data(), V) != 0)
    { sodium_memzero(&st, sizeof(st)); return -1; }
